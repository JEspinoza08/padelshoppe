    ...
    >
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 rounded-2xl border border-amber-400/20 bg-amber-400/5 p-4">
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={Boolean(values.is_preorder)}
                onChange={(e) => setField("is_preorder", e.target.checked)}
                className="mt-1"
              />
              <div>
                <p className="font-black text-white">Producto en preventa</p>
                <p className="text-xs text-gray-400">Disponible para cualquier marca. El cliente reserva online y recoge/paga el saldo en tienda.</p>
              </div>
            </label>
            {values.is_preorder && (
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Field label="Porcentaje a pagar para reservar">
                  <input type="number" min="1" max="100" value={values.preorder_percentage || 50} onChange={(e) => setField("preorder_percentage", Number(e.target.value))} className="input-admin" />
                </Field>
                <Field label="Mensaje de preventa">
                  <input value={values.preorder_note || ""} onChange={(e) => setField("preorder_note", e.target.value)} placeholder="Ej: Entrega estimada: enero 2027" className="input-admin" />
                </Field>
              </div>
            )}
          </div>
