...
export default function CheckoutPage() {
  ...
  const preorderDeposit = cart.reduce((sum, item) => {
    if (!item.isPreorder) return sum;
    return sum + item.price * item.quantity * ((item.preorderPercentage || 50) / 100);
  }, 0);
  const regularSubtotal = cart.reduce((sum, item) => item.isPreorder ? sum : sum + item.price * item.quantity, 0);
  const payableSubtotal = regularSubtotal + preorderDeposit;
  const effectiveShippingCost = hasPreorder ? 0 : shippingCost;
  const finalTotal = payableSubtotal + effectiveShippingCost;
  const outstandingBalance = Math.max(0, totalPrice - payableSubtotal);
  ...
  useEffect(() => {
    if (hasPreorder) { setShippingCost(0); return; }
    ...
    getShippingRate(department, province, district)
      ...
      .catch(console.error);
  }, [department, province, district, hasPreorder]);
  ...
  const validateCheckout = () => {
    ...
    if (
      ...
      !dni ||
      !phone ||
      (!hasPreorder && (!department || !province || !district || !address.trim()))
    ) {
      setToast({
        type: "warning",
        title: "Datos incompletos",
        message:
          hasPreorder ? "Completa nombre, DNI y celular." : "Completa nombre, DNI, celular, departamento, provincia, distrito y dirección.",
      });
      return false;
    }
    ...
    if (!hasPreorder && address.trim().length < 8) {
      setToast({
        type: "warning",
    ...
    try {
      const orderResult = await createOrder({
        ...
        customer: {
          ...
          dni,
          phone,
          address: hasPreorder ? "Recojo en tienda" : address,
          department: hasPreorder ? "Lima" : department,
          province: hasPreorder ? "Lima" : province,
          district: hasPreorder ? "Recojo en tienda" : district,
          reference,
        },
        paymentMethod,
        cart: cart.map((item) => ({ ...item, price: item.isPreorder ? item.price * ((item.preorderPercentage || 50) / 100) : item.price })),
        subtotal: payableSubtotal,
        shippingCost: effectiveShippingCost,
        totalPrice: finalTotal,
        fulfillmentType: hasPreorder ? "store_pickup" : "delivery",
        isPreorder: hasPreorder,
        fullRetailTotal: totalPrice,
        outstandingBalance,
        paymentData,
      });
      ...
      const { data, error } = await supabase.functions.invoke(
        "create-payment",
        {
          body: {
            ...
            email: user.email,
            source_id: tokenId,
            description: `${hasPreorder ? "Reserva preventa" : "Compra"} PadelShop - ${name}`,
            installments: Number(
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_420px]">
          <form className="rounded-2xl border border-neutral-200 p-6">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              {hasPreorder ? (
                <div className="md:col-span-2 rounded-2xl border border-[#f04b2f]/25 bg-[#f04b2f]/5 p-4">
                  <p className="font-black text-neutral-950">Recojo en tienda seleccionado automáticamente</p>
                  <p className="mt-1 text-sm text-neutral-600">Tu pedido contiene una preventa. No se cobrará envío y el saldo pendiente se cancela al recoger el producto en tienda.</p>
                </div>
              ) : <>
              <CustomSelect
                value={department}
            ...
            >
              {isProcessingPayment ? "Validando..." : hasPreorder ? `Reservar pagando S/ ${finalTotal.toLocaleString("es-PE")}` : "Confirmar pedido"}
            </button>
          </form>
          ...
          <aside className="rounded-2xl border border-neutral-200 p-6">
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <strong className="mt-1 block text-sm">
                          Precio total: S/ {(item.price * item.quantity).toLocaleString("es-PE")}
                        </strong>
                        {item.isPreorder && <p className="mt-1 text-xs font-bold text-[#f04b2f]">Reserva hoy: S/ {(item.price * item.quantity * ((item.preorderPercentage || 50) / 100)).toLocaleString("es-PE")}</p>}
                      </div>
            ...
            <div className="mt-6 space-y-3 border-t pt-5">
              ...
              <div className="flex justify-between">
                ...
                <strong>{hasPreorder ? "Recojo en tienda · S/ 0" : `S/ ${effectiveShippingCost.toLocaleString("es-PE")}`}</strong>
              </div>

              {hasPreorder && <div className="flex justify-between text-[#f04b2f]"><span className="font-bold">Saldo al recoger</span><strong>S/ {outstandingBalance.toLocaleString("es-PE")}</strong></div>}
              <div className="flex justify-between border-t pt-4">
                <span className="font-black">{hasPreorder ? "Pagar ahora" : "Total"}</span>
