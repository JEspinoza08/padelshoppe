...
>
          ...
          <div className="p-6 lg:p-10">
            ...
            {product.isPreorder && (
              <div className="mt-5 rounded-2xl border border-[#f04b2f]/25 bg-[#f04b2f]/5 p-4">
                <p className="text-xs font-black uppercase text-[#f04b2f]">Preventa</p>
                <p className="mt-1 text-sm font-bold text-neutral-900">Precio total: S/ {product.price.toLocaleString("es-PE")}</p>
                <p className="mt-1 text-sm text-neutral-600">Resérvalo pagando {product.preorderPercentage || 50}%: <strong>S/ {(product.price * ((product.preorderPercentage || 50) / 100)).toLocaleString("es-PE")}</strong>. El saldo se paga al recoger en tienda.</p>
                {product.preorderNote && <p className="mt-2 text-xs text-neutral-500">{product.preorderNote}</p>}
              </div>
            )}
            <div className="mt-3">
              {hasStock ? (
...
if (product.isPreorder) {
  ...
  !hasStock ? "cursor-not-allowed opacity-50" : ""
              ...
              >
                ...
                {hasStock ? (product.isPreorder ? "Realizar la reserva" : "Agregar al carrito") : "Sin stock"}
              </button>
            </div>
          </div>
        </div>
