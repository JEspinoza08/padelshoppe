...
export default function ProductsSection({
  ...
  const filtered = useMemo(() => {
     ...
     * 2. Preventas aunque tengan stock físico 0.
     * 3. Productos realmente agotados al final.
     * 4. Dentro de cada grupo, NOX primero.
     * 5. Luego:
     *    palas -> bolsos -> zapatillas -> ropa -> accesorios.
     */
    ...
    const isOutOfStock = (product: Product) => {
      // Las preventas están disponibles para reservar aunque
      // todavía no tengan stock físico.
      if (product.isPreorder) {
        return false;
      }

      // Productos con variantes:
      // basta que una variante activa tenga stock para
      // considerar el producto disponible.
    ...
    return [...result].sort((a, b) => {
      ...
      const aBrandPriority =
        a.brand.trim().toUpperCase() === "NOX" ? 0 : 1;

      const bBrandPriority =
        b.brand.trim().toUpperCase() === "NOX" ? 0 : 1;

      if (aBrandPriority !== bBrandPriority) {
        return aBrandPriority - bBrandPriority;
      }
      ...
      /*
       * Si tienen la misma prioridad mantenemos
       * el orden original de getProducts().
       */
