...
export default function ProductsSection({
  ...
  const filtered = useMemo(() => {
    ...
    // Prioridad de destacados:
    // 1) NOX siempre aparece antes que el resto de marcas.
    // 2) Dentro de cada bloque de marca, el tipo de producto se ordena así:
    //    palas -> bolsos -> zapatillas -> ropa -> accesorios.
    // 3) Si dos productos tienen la misma marca/prioridad y categoría,
    //    se conserva el orden original que trae getProducts() (más recientes primero).
    ...
    return [...result].sort((a, b) => {
      const aBrandPriority = a.brand.trim().toUpperCase() === "NOX" ? 0 : 1;
      const bBrandPriority = b.brand.trim().toUpperCase() === "NOX" ? 0 : 1;
